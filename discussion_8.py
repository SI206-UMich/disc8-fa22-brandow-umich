...
def getLink(soup):
    tag = soup.find('a',
                    title='List of American universities with Olympic medals')
    info = "https://en.wikipedia.org" + tag.get('href')
    return info


# Task 3: Get the details from the box titled "College/school founding". Get all the college/school names and the
# year they were founded and organize the same into key-value pairs.
def getAdmissionsInfo2019(soup):
    ...
    for tr in list[1:]:
        list = tr.find_all('td')
        sch_name = list[0].text.strip()
        founded = list[1].text.strip()
        d[sch_name] = founded
    return d


def main():
    # Task 1: Create a BeautifulSoup object and name it soup. Refer to discussion slides or lecture slides to
    # complete this
...
class TestAllMethods(unittest.TestCase):
    def setUp(self):
        self.soup = BeautifulSoup(requests.get('https://en.wikipedia.org/wiki/University_of_Michigan').text,
                                  'html.parser')

    def test_link_nobel_laureates(self):
        self.assertEqual(getLink(self.soup),
